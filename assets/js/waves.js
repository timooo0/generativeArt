...
function createWaves(startX, width, numWaves) {
    ...
    for (let waveId = 0; waveId <= numWaves; waveId++){
        ...
        for (let i=0; i < width; i += 1){
            if (waveId != numWaves){
                wavefront.push(waveSize*noise((width*waveId+i)/200)+offset+waveSize*waveId)
            } else{
    ...
    for (let waveId = 0; waveId < numWaves; waveId++){
        ...
        for (let i=0; i < width; i = i + squareSize){
            let x = 0
            for (let j=Math.floor(waves[waveId][i]); j <= Math.ceil(waves[waveId+1][i]); j = j + squareSize){
                let bValue = baseBlue + (deltaBlue*4)*(x/((waves[waveId+1][i]-waves[waveId][i])/squareSize))**2
