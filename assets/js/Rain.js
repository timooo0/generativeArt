...
function setup() {
  ...
  for (let i=0;i<canvasX*rainDensity;i++){
    for (let j=0;j<canvasY*rainDensity;j++){
      rain.push([Math.random()*canvasX, // x position
  ...
  for (let i=0;i<rain.length;i++){
    ...
    for (const blocker of blockers){
      if (rain[i][0] > blocker[0] && rain[i][0] < blocker[0]+2*blocker[1]){
        if (rain[i][1]+rain[i][2] > -0.5*Math.sqrt(blocker[1]**2 - ((rain[i][0]-blocker[0]-blocker[1]))**2)+blocker[2]){
          stroke(rainColor2);
        }
      }
    }
...
function updateObjects(){
  ...
  for (const blocker of blockers){
    blocker[0] += blocker[3];
    blocker[2] = blocker[3]*10*Math.cos(blocker[0]*Math.PI/(canvasX/10))**2+blocker[4];
  }
...
function drawPerson() {
  for (const blocker of blockers){
    ...
    noStroke();
    fill(c);
    arc(blocker[0]+blocker[1], blocker[2], 2*blocker[1], 0.5*2*blocker[1], PI, 0);
    
    stroke(c);
    line(blocker[0]+blocker[1], blocker[2], blocker[0]+blocker[1], blocker[2]+blocker[1]);

    noFill();
    strokeWeight(rainWidth*2);
    line(blocker[0]+blocker[1], blocker[2]+0.8*blocker[1], blocker[0]+blocker[1], blocker[2]+blocker[1])
    arc(blocker[0]+blocker[1]+0.1*blocker[1], blocker[2]+blocker[1], 0.2*blocker[1], 0.2*blocker[1], 0, PI);
  }
}
