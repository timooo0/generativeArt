...
function changeColor(originalColor) {
  ...
  for (const index of indexArray) {
    [temp, originalColor] = splitStringAt(
      originalColor,
      originalColor.indexOf(index)
    );
    newColor.push(temp);
  }
  let rgbIndex = Math.floor(Math.random() * 3 + 1);
  let orignalValue = Number(
    newColor[rgbIndex].substring(0, newColor[rgbIndex].indexOf(","))
  );
...
function splitrectVert(startrect, splitRatio) {
  let newrect1 = [
    startrect[0],
    startrect[1],
    startrect[2] * splitRatio,
    startrect[3],
  ];
  let newrect2 = [
    startrect[0] + startrect[2] * splitRatio + offset,
    startrect[1],
    startrect[2] * (1 - splitRatio) - offset,
    startrect[3],
  ];
...
function splitrectHor(startrect, splitRatio) {
  let newrect1 = [
    ...
    startrect[1],
    startrect[2],
    startrect[3] * splitRatio,
  ];
  let newrect2 = [
    startrect[0],
    startrect[1] + startrect[3] * splitRatio + offset,
    startrect[2],
    startrect[3] * (1 - splitRatio) - offset,
  ];
  return [newrect1, newrect2];
}
...
function draw() {
  ...
  if (
    (finalSplitRatio < 0.5 && splitRatio <= finalSplitRatio - epsilon) ||
    (finalSplitRatio >= 0.5 && splitRatio >= finalSplitRatio + epsilon)
  ) {
